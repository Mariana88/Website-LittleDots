<?php
	...
	$res_main = DBConnect::query("(SELECT `id`, `date`, 'n' as `type` FROM data_news, data_news_lang WHERE data_news.id=data_news_lang.lang_parent_id AND data_news_lang.lang='NL' AND published='1') UNION (SELECT `id`, `date`, 'c' as `type` FROM data_concert, data_concert_lang WHERE data_concert.id=data_concert_lang.lang_parent_id AND data_concert_lang.lang='NL' AND published='1' AND `date` > '" . (time()-86400) . "') ORDER BY `type` DESC, `date` DESC" ,__FILE__, __LINE__);
	...
	while($row_main = mysql_fetch_array($res_main))
	{
		if($row_main["type"] == "n")
		{
			$res = DBConnect::query("SELECT * FROM data_news, data_news_lang WHERE data_news.id=data_news_lang.lang_parent_id AND data_news_lang.lang='NL' AND `id`='" . $row_main["id"] . "'", __FILE__, __LINE__);
			$row = fetch_db($res);
			$item = $feed->add( 'item' );
			...
			$link = $item->add( 'link' );
			$link->href = 'http://www.deberengieren.be/NL/Home/n' . $row["id"]; 
		}
		
		if($row_main["type"] == "c")
		{
			$res = DBConnect::query("SELECT * FROM data_concert, data_concert_lang WHERE data_concert.id=data_concert_lang.lang_parent_id AND data_concert_lang.lang='NL' AND `id`='" . $row_main["id"] . "'", __FILE__, __LINE__);
			$row = fetch_db($res);
			$item = $feed->add( 'item' );
			...
			$link = $item->add( 'link' );
			$link->href = 'http://www.deberengieren.be/NL/Agenda/c' . $row["id"]; 
		}
	}
